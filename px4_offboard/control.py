...
Press T to start SINE trajectory (climb to 30m then sine pattern)
Press Y to start CIRCLE trajectory (climb to 30m then circle pattern)
Press R to RESET (return to manual control)
"""
...
def main():
    ...
    # Publisher for trajectory mode
    trajectory_mode_pub = node.create_publisher(String, '/trajectory_mode', qos_profile)
    ...
    try:
        ...
        while True:
            ...
            # Handle trajectory mode keys first
            if key == 't' or key == 'T':
                mode_msg = String()
                mode_msg.data = "SINE"
                trajectory_mode_pub.publish(mode_msg)
                print("Starting SINE trajectory (will climb to 30m first)")
            ...
            if key == 'y' or key == 'Y':
                ...
                trajectory_mode_pub.publish(mode_msg)
                print("Starting CIRCLE trajectory (will climb to 30m first)")
                x_val = y_val = z_val = yaw_val = 0.0
                # Publish zero velocity
                twist = geometry_msgs.msg.Twist()
                twist.linear.x = 0.0
            ...
            # Publish velocity for manual control
            twist = geometry_msgs.msg.Twist()
